...
class MachinesController extends Controller
{
    ...
    public function searchList(Request $request)
    {
        ...
        ->first();
        $data = hours::where([['machine_name', 'like', '%' . $request->machine_name . '%'], ['hours', 'like', '%' . $request->type . '%']])
            ->first();
        if ($data == null) {
            return redirect()->back()->with('error', 'The checklist has not been added yet');
        } elseif ($data->hours == 'Weekly' || $data->hours == 'Daily') {
    ...
    public function storeReport(Request $request, $id)
    {
        ...
        Mail::to('lawrence@betterglobeforestry.com')->send(new Notify(auth()->user()));
    ...
    public function updateD(Request $request, $id)
    {
        ...
        if ($request->type == 'Weekly' || $request->type == 'Daily') {
            $machine->update([
